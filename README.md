...
Также, `Containy` создаёт `overlayFS`, этот тип файловой системы используется в `Docker`. Она накладывает один “слой” файлов на другой, позволяя создавать общее дерево каталогов. Данный подход помогает экономить место на диске, ведь многие контейнеры могут иметь одну и ту же базовую файловую систему, например, `ubuntu` или `debian`. Поэтому мы храним лишь один “снапшот” этой базовой файловой системы, а изменения, которые контейнер вносит в них, сохраняем отдельно для каждого из контейнеров. При повторном запуске контейнера эти изменения накладываются поверх базовой файловой системы.
...
Итак, мы разрешаем контейнеру создать максимум 10 дочерних процессов и выделать не более, чем 512 Мегабайт оперативной памяти
